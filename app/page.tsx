...
export default function Home() {
  ...
  const clearProject = () => { setProjectFiles([]); setProjectName(""); setNotice("Project context cleared."); };
  ...
  function handleImages(fileList: FileList | null) {
    ...
    setNotice("Images attached. Nemotron 3 Ultra is currently a text-output model, so these images are shown as references but are not visually analyzed by the model.");
  }
  ...
  async function sendMessage(customPrompt?: string) {
    ...
    try {
      const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: updated, thinking, projectContext: buildProjectContext() }) });
      ...
      while (true) {
        ...
        for (const part of parts) {
          ...
          if (data.type === "content") setMessages(current => { const copy = [...current]; const last = copy.length - 1; copy[last] = { role: "assistant", content: copy[last].content + data.content }; return copy; });
          if (data.type === "error") throw new Error(data.error);
        }
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unknown error";
      setMessages(current => { const copy = [...current]; copy[copy.length - 1] = { role: "assistant", content: `❌ **Error:** ${message}` }; return copy; });
    } finally { setLoading(false); }
  }

  async function copyCode(text: string) { await navigator.clipboard.writeText(text); }
  const suggestions = [["🐍 Build Python App", "Create a complete Python task manager application."], ["⚛️ React Component", "Create a modern React dashboard component using Tailwind CSS."], ["🐛 Fix My Error", "Help me debug this code. Explain the error and provide the fixed version:\n\n"], ["🌐 Build Website", "Create a modern responsive landing page using HTML, CSS and JavaScript."]];
