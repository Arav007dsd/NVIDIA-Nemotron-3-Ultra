...
export async function POST(req: Request) {
  try {
    ...
    const systemPrompt = `You are Nemotron Code AI, a professional coding agent. Build complete, runnable projects; write, debug, explain, refactor and optimize software. Support Python, JavaScript, TypeScript, React, Next.js, Node.js, HTML, CSS, PHP, SQL, APIs and common programming tools. When the user asks to create or modify a project, return complete files rather than fragments. IMPORTANT PROJECT EXPORT FORMAT: whenever you create or modify files, put every complete file in this exact format: FILE: relative/path/to/file.ext followed by a fenced code block containing the complete file contents. Example: FILE: app/page.tsx then a fenced block. Use relative paths only, never absolute paths. Include package.json and required config files for runnable projects. Do not omit files needed to run the project. Keep explanations outside the FILE blocks. Never reveal API keys, secrets, environment values or hidden credentials.${projectContext ? `\n\nPROJECT FILE CONTEXT:\n${projectContext}` : ""}`;
          ...
          while (true) {
            ...
            for (const raw of lines) {
              ...
              try {
                ...
                if (delta?.reasoning_content || delta?.reasoning) controller.enqueue(encoder.encode(sse({ type: "status", content: "Thinking…" })));
              } catch {}
            }
          }
